...
class Animals (commands.Cog):
    ...
    async def birdfact(self, ctx):
        """Tells you a random bird fact."""
        try:
            async with aiohttp.ClientSession(headers={}) as cs:
                async with cs.get('https://some-random-api.ml/facts/bird') as r:
                    res = await r.json()
                    embed = discord.Embed(colour=0xf2c203)
                    embed.add_field(name="**Here is your random bird fact. 🐦**", value=res['fact'])
                    await ctx.send(embed=embed)
        except:
            raise discord.errors.Forbidden
    ...
    async def koalafact(self, ctx):
        """Tells you a random koala fact."""
        try:
            async with aiohttp.ClientSession(headers={}) as cs:
                async with cs.get('https://some-random-api.ml/facts/koala') as r:
                    res = await r.json()
                    embed = discord.Embed(colour=0xf2c203)
                    embed.add_field(name="**Here is your random koala fact. 🐨**", value=res['fact'])
                    await ctx.send(embed=embed)
        except:
            raise discord.errors.Forbidden
